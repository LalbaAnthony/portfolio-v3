...
export function useAppHead(options: HeadOptions = {}) {
    ...
    // Delete all key of option where the value is null or undefined or empty string to default more easily
    Object.keys(options).forEach(key => {
        const value = options[key as keyof HeadOptions]
        if (value === null || value === undefined || value === '') {
            // eslint-disable-next-line @typescript-eslint/no-dynamic-delete
            delete options[key as keyof HeadOptions]
        }
    })
